...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       //am creat graful
        ...
        mst= primAlg(graph, nodeList.size());
        //cu primAlg aplicam algoritmul lui Prim pe graful creat anterior
        getMetricTSP(mst);
        RandomProblemGenerator newGraph=new RandomProblemGenerator();
        newGraph.generate();
        //newGraph.toString();
    }
    ...
    public static List<String> primAlg(SimpleDirectedWeightedGraph<String, DefaultWeightedEdge> graph,int nrNodes){
      ...
      List<String> mst=new ArrayList<>(); //lista de varfuri in ordinea in care au fost parcurse de algoritm
        ...
        adjLists = new LinkedList[nrNodes];
        for (int i = 0; i <nrNodes ; i++) {
            adjLists[i] = new LinkedList<Integer>();
        }//lista de adiacente pt DFS
        visitedDFS = new boolean[nrNodes];
                 ...
                 for (DefaultWeightedEdge i : graph.edgesOf(j)) {
                     if (costMin == -1 || graph.getEdgeWeight(i) < costMin) {
                         if (visited.get(graph.getEdgeTarget(i)) == false) {
                             ...
                             edgeList.add(i);
                             int s=Integer.parseInt(String.valueOf(graph.getEdgeSource(i).charAt(1)));
                             int t=Integer.parseInt(String.valueOf(graph.getEdgeTarget(i).charAt(1)));
                             adjLists[s].add(t);
                         }
                     }
                 }
    ...
    public static void getMetricTSP(List<String> mst){
            DFS(0);

            System.out.println("Pentru radacina " + mst.get(0) + " circuitul este: ");
            for (int j = 0; j < solution.size(); j++) {
                System.out.println(mst.get(solution.get(j)) );
            }
            System.out.println(mst.get(solution.get(0)));
